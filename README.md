...
*Багато користувачів починають із додавання всієї логіки до ***gulpfile.babel.js***. Якщо він стане занадто великим, його можна переоформити в окремі файли які будуть перетворені на окремі функції і занесені в папку ***tasks***.* *Допоміжний вузол для ```require()``` каталогів. Файли каталогу перевіряються, і кожен з них, який можна require()"d", require()"d" і повертається як частина хешу від базового імені цього файлу до його експортованого вмісту.
